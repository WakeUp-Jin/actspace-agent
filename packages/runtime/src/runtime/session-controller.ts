...
export class RuntimeSessionController {
  ...
  #browseIndex?: SessionBrowseIndex;
  private get browseIndex() {
    return this.#browseIndex ??= new SessionBrowseIndex(this.options.dataRoot, async (id) => {
      const live = this.#open.get(id);
      if (live) return { snapshot: this.snapshot(live), journal: live.journal.events, profileId: live.header.createdWith.profileId };
      const inspection = await this.store.inspect(id);
      if (!inspection.header || !inspection.validation) throw new Error('Cannot index damaged session');
      return { snapshot: { ...projectSessionSnapshot({ header: inspection.header, events: inspection.events, registry: this.options.registry, rendererAllowlist: this.options.rendererAllowlist }), accessState: inspection.accessState }, journal: inspection.events, profileId: inspection.header.createdWith.profileId };
    }, this.options.registry.digest);
  }
  browseToolDetail(sessionId: string, callId: string) { return this.browseIndex.detail(sessionId, callId); }
  browseSession(sessionId: string, before?: number) { return this.browseIndex.page(sessionId, before); }
  async browseSessions() {
    const root = `${this.options.dataRoot}/sessions-v2`;
    await mkdir(root, { recursive: true });
    const ids = (await readdir(root, { withFileTypes: true })).filter(e => e.isDirectory()).map(e => e.name);
    const items: RuntimeV2SessionListItem[] = [];
    const missing: string[] = [];
    for (const id of ids) { const cached = await this.browseIndex.cached(id); if (cached) items.push(cached.item); else missing.push(id); }
    if (missing.length > 0 && !this.#indexing) {
      this.#indexing = true;
      void (async () => {
        for (const id of missing) {
          if (this.#closing) break;
          try { await this.browseIndex.ensure(id); this.#indexErrors.delete(id); }
          catch { this.#indexErrors.add(id); }
          await new Promise<void>(resolve => setImmediate(resolve));
        }
      })().finally(() => { this.#indexing = false; });
    }
    return { items, indexing: missing.some(id => !this.#indexErrors.has(id)), failed: this.#indexErrors.size };
  }
  #indexing = false;
  #closing = false;
  ...
  async list(): Promise<readonly RuntimeV2SessionListItem[]> {
    const root = `${this.options.dataRoot}/sessions-v2`; await mkdir(root, { recursive: true });
    const results: RuntimeV2SessionListItem[] = [];
    for (const entry of await readdir(root, { withFileTypes: true })) {
      if (!entry.isDirectory()) continue;
      try { results.push((await this.browseIndex.ensure(entry.name)).item); } catch { /* damaged sessions stay out of normal navigation */ }
    }
    return results.sort((a, b) => b.updatedAt.localeCompare(a.updatedAt) || a.sessionId.localeCompare(b.sessionId));
  }
