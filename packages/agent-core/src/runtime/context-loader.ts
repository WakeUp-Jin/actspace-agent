...
import type { AgentRuntimeContext, AgentSystemPromptSegment } from "../engine/create-agent-deps";
import { createSkillCatalogSegment, loadSkillRegistry, parseSkillFile } from "../skills";

type WarningLogger = (message: string, details?: Record<string, unknown>) => void;

export type LoadAgentRuntimeContextInput = {
  dataRoot: string;
...
export async function loadAgentRuntimeContext(
  input: LoadAgentRuntimeContextInput,
): Promise<Pick<
  AgentRuntimeContext,
...
>> {
  ...
  if (mode === "agent") {
    systemPromptSegments.push(createMainAgentKairosHandoffSegment(join(kairosInboxRoot, "main-agent.md")));
  }
  ...
  const browserBridgeRuntime = mode !== "agent" || disabledTools.has("browser") || disabledTools.has("browser_help")
    ? undefined
    : await resolveBrowserBridgeRuntime(input.browserBridgeAbbPath, input.browserBridgeSocketPath);
  if (browserBridgeRuntime) systemPromptSegments.push(browserBridgeRuntime.segment);
  ...
  const filteredRegistry = disabled.size === 0
    ? skillRegistry
    : { ...skillRegistry, skills: skillRegistry.skills.filter((skill) => !disabled.has(skill.name)) };
  const skillCatalogSegment = mode === "chat" ? undefined : createSkillCatalogSegment(filteredRegistry);
  if (skillCatalogSegment) systemPromptSegments.push(skillCatalogSegment);

  if (mode === "chat") systemPromptSegments.push(createChatModeSegment());
  if (mode === "plan") systemPromptSegments.push(createPlanModeSegment());
...
export async function loadAgentsMdSegments(
  ...
  const sources = [
    {
      ...
      path: join(input.dataRoot, "AGENTS.md"),
      priority: 90,
    },
    {
      id: "agents_workspace",
      title: "Workspace AGENTS.md",
      path: join(input.workspaceRoot, "AGENTS.md"),
      priority: 80,
    },
  ];
  ...
  for (const source of sources) {
    let content: string;
    try {
      content = await readTextFile(source.path);
    } catch (error) {
      if (isNotFoundError(error)) continue;
      input.warn?.("optional text file read failed", {
...
async function createSelectedSkillSegments(input: {
  ...
  for (const name of selectedNames) {
    ...
    if (!skill) throw new Error(`Selected Skill is not available: ${name}`);
    const parsed = parseSkillFile(await readFile(skill.location, "utf8"), skill.directory);
    if (parsed.warning) throw new Error(`Selected Skill cannot be loaded: ${name}`);
    segments.push({
