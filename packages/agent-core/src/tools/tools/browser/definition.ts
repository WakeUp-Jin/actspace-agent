...
export const browserDomDefinition: ToolDefinitionSpec = {
  ...
  parameters: {
    ...
    properties: {
      ...
      text: { type: "string", description: "要输入的文本。" },
      keys: { type: "array", items: { type: "string" }, description: "按键或组合键。" },
      limit: { type: "number", description: "snapshot 最大节点数；默认 500，硬上限 1000。" },
    },
    required: ["action", "tab_id"],
    additionalProperties: false,
  },
...
export const browserLocatorDefinition: ToolDefinitionSpec = {
  ...
  parameters: {
    ...
    properties: {
      ...
      offset: { type: "number", description: "all_text_contents/read_all 的零基分页偏移。" },
      limit: { type: "number", description: "all_text_contents/read_all 的分页数量；默认 200，硬上限 1000。" },
