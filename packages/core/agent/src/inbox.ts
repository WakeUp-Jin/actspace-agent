...
export type InboxItem = { readonly messageId: string; readonly target: InboxTarget; readonly content: RuntimeV2JsonValue; readonly enqueuedSeq: number };
...
export class MainAgentInbox {
  ...
  async enqueue(content: RuntimeV2JsonValue, target: InboxTarget, messageId: string = randomUUID()): Promise<InboxItem> {
    const event = await this.session.append(core("agent/inbox/spliced", { operation: "enqueue", messageId, target, content }));
  ...
  pending(): readonly InboxItem[] {
    ...
    for (const event of this.session.journal.events) {
      ...
      if (event.type === "agent/inbox/spliced" && data.operation === "enqueue") pending.set(messageId, { messageId, target, content: data.content ?? null, enqueuedSeq: event.seq });
  ...
  async claim(target: InboxTarget, limit = target === "next-turn" ? 1 : Number.POSITIVE_INFINITY, options: InboxClaimOptions = {}): Promise<readonly InboxItem[]> {
    ...
    for (const item of items) {
      ...
      await this.session.append(core("agent/inbox/spliced", { operation: "claim", messageId: item.messageId, target }, surface));
    }
