...
export function toolPreview(toolName: string, tool: RuntimeV2ToolView | undefined, data: EventRecord, call: EventRecord | undefined, child: { childSessionId: string; presetId: string; status: string } | undefined, sessionId: string, agentRunId: string, phase: "streaming" | "prepared" | "running" | "finished" = "finished", workspaceRoot?: string, fileChangeStats?: FileChangeStats): ToolUiPreview {
  ...
  const resultPreview = terminal && !failed ? outputPreview(output) : undefined;
  if (localName === "read_file") return { kind: "read", filePath: displayPath(string(args.path) ?? ""), ...(typeof args.offset === "number" || typeof args.limit === "number" ? { range: `${nonNegative(args.offset) || 1}-${(nonNegative(args.offset) || 1) + Math.max(0, nonNegative(args.limit) - 1)}` } : {}), displayText: summary, resultPreview };
  ...
  if (localName === "write_file" || localName === "edit_file") {
    ...
    const additions = terminal ? nonNegative(result.additions) : fileChangeStats?.additions ?? 0;
    const deletions = terminal ? nonNegative(result.deletions) : fileChangeStats?.deletions ?? 0;
    const outputMetadata = terminal && !failed ? workspaceOutputMetadata(string(result.path) ?? string(args.path) ?? "", workspaceRoot) : {};
    const common = { filePath, additions, deletions, diff: terminal && !failed ? output.split("\n\nFile ", 1)[0] ?? "" : "", collapsedLines: 0, status: fileStatus, ...outputMetadata, ...(errorMessage ? { errorMessage } : {}) };
  ...
  if (["agent", "explore", "actspace.subagent/agent", "actspace.subagent/explore"].includes(toolName)) { const detail = record(detailValue(data, "delegation")); const childSessionId = string(detail.childSessionId) ?? child?.childSessionId ?? ""; const presetId = string(detail.presetId) ?? child?.presetId ?? (toolName.endsWith("explore") ? "actspace.explore" : "actspace.agent"); const status = string(detail.status) === "aborted" || child?.status === "aborted" || state === "aborted" ? "aborted" : child?.status === "failed" || failed ? "failed" : !terminal ? "running" : "completed"; return { kind: "agent", description: string(args.task) ?? (toolName.endsWith("explore") ? "Explore" : "Agent"), status, subagentType: "explore", agentKind: presetId === "actspace.explore" || toolName.endsWith("explore") ? "explore" : "agent", displayText: summary, summary: output, ...(childSessionId ? { transcriptRef: { kind: "subagent_transcript", sessionId, agentRunId, runId: childSessionId } } : {}), stats: { durationMs: tool?.durationMs ?? 0, toolCallCount: 0 }, display: "panel", ...(errorMessage ? { error: errorMessage } : {}) }; }
...
function outputPreview(output: string): string[] | undefined { const lines = output.split("\n").map((line) => line.trim()).filter(Boolean).slice(0, 8).map((line) => line.length > 240 ? `${line.slice(0, 237)}…` : line); return lines.length ? lines : undefined; }
...
function workspaceOutputMetadata(path: string, workspaceRoot?: string): { outputPath?: string; outputRelativePath?: string } {
  if (!path || !workspaceRoot) return {};
  const absolute = resolve(workspaceRoot, path);
  const local = relative(workspaceRoot, absolute);
  if (local === ".." || local.startsWith(`..${sep}`) || isAbsolute(local)) return {};
  return { outputPath: absolute, outputRelativePath: local || "." };
}
