...
var registry = []Command{
	...
	// DOM CUA: 7
	command(commandSpec{id: "dom_cua_get_visible_dom", category: "dom", action: "snapshot", description: "Return visible interactive DOM nodes with stable node ids.", input: tabOnlySchema(), output: object([]string{"nodes"}, map[string]Schema{"nodes": arrayProperty("Visible DOM nodes.", object(nil, map[string]Schema{}))}), risk: RiskLow, readOnly: true, status: StatusImplemented, handler: "go.dom.snapshot"}),
	...
	command(commandSpec{id: "playwright_locator_all_text_contents", category: "locator", action: "all_text_contents", description: "Read textContent from every matching element.", input: selectorSchema(nil), output: object([]string{"values"}, map[string]Schema{"values": arrayProperty("Text values.", stringProperty("Text content."))}), risk: RiskLow, readOnly: true, status: StatusImplemented, handler: "go.locator.all_text_contents"}),
	command(commandSpec{id: "playwright_locator_read_all", category: "locator", action: "read_all", description: "Read attributes and text from every matching element.", input: selectorSchema(map[string]Schema{"relative_selector": stringProperty("Optional selector evaluated relative to each match.")}), output: object([]string{"values"}, map[string]Schema{"values": arrayProperty("Element records.", object(nil, map[string]Schema{}))}), risk: RiskLow, readOnly: true, status: StatusImplemented, handler: "go.locator.read_all"}),
	...
	command(commandSpec{id: "playwright_scroll", category: "locator", action: "scroll", description: "Scroll the page or a located element.", input: object([]string{"tab_id", "direction"}, map[string]Schema{"tab_id": tabID, "selector": selector, "direction": enumProperty("Scroll direction.", "up", "down", "left", "right"), "amount": numberProperty("Scroll amount in CSS pixels.")}), output: emptyResult, risk: RiskMedium, status: StatusImplemented, handler: "go.locator.scroll", aliases: []string{"browser_scroll"}}),
