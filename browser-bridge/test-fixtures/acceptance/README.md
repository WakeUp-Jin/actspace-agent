...
```bash
node browser-bridge/test-fixtures/acceptance/smoke.cjs
```
...
```bash
node browser-bridge/test-fixtures/acceptance/io-smoke.cjs
```
...
```bash
node browser-bridge/test-fixtures/acceptance/claim-smoke.cjs
```

To verify the Runtime v2 Tool policy, Host approval, durability checkpoint, and denial path against Chrome:

```bash
node browser-bridge/test-fixtures/acceptance/agent-approval-smoke.cjs
```

To verify deliverable finalization and ownership release:

```bash
node browser-bridge/test-fixtures/acceptance/deliverable-smoke.cjs
```

To verify that one Agent browser session cannot finalize another session's handoff tabs:

```bash
node browser-bridge/test-fixtures/acceptance/session-isolation-smoke.cjs
```
