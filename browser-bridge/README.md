...
This plugin is a **host-bridge plugin**. Browser control requires Chrome Native Messaging, a local socket, and extension permissions, so it remains a Host capability rather than an in-process Cordis code plugin.
...
The build writes the `abb` binary to `browser-bridge/skill/scripts/abb`.
