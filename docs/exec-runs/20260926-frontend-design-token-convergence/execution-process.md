...
- Electron 窗口验收：代码实施结束时暂未执行；2026-09-26 后续已通过 Computer Use 部分验收，详见下方补充记录。
...
## 2026-09-26 Computer Use 验收补充

- 按「验收点清单 + 场景分组」执行，结果见 [computer-use-acceptance.md](computer-use-acceptance.md)。模型相关验收按用户要求延期。
- 首轮覆盖主界面、输入/按钮、审批拒绝及非模型设置/扩展；发生 EPIPE 后中断。恢复 `pnpm dev:log` 后，以工作区/附件/窄屏、系统外观/动效/弹窗两个路线补验。
- 附件添加与移除、窄屏抽屉及标题栏操作、跟随系统浅深双向变化通过。图片生成配置弹窗的 Tab 焦点落背景、Esc 不关闭可复现；源码有焦点陷阱与 Escape 处理，根因尚未确认。
- 浮层真实相交、运行中减少动态效果、模型设置及部分精确尺寸未完整覆盖，不标整体验收通过。
- 已恢复系统与应用外观、减少动态效果及宽窗口，移除临时草稿附件；验收操作阶段只同步文档，没有修改产品代码。
