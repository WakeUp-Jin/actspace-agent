...
- `docs/archive/v1/design-docs/agent-runtime-current-module-map.md` —— 新增 "## `kairos/` - 自治模式" 章节，含 mermaid 模块依赖图 + 各子模块速读 + desktop 集成位点。
- `docs/design-docs/core-storage-and-observability.md` —— 新增 "## Kairos 存储与可观测性"，含目录树、4 个 SessionEvent 新类型、IPC 通道表、排障日志归属。
- `docs/archive/v1/design-docs/agent-kairos-autonomous-mode.md` —— 顶部状态徽章改为 "v1 代码已上线"；末尾追加 "## 附录：v1 实测目录树"。
