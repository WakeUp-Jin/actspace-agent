...
- 新增 `docs/archive/v1/design-docs/execution-safety-agent-bash工具设计文档.md`：当时的 Bash 工具设计事实来源——工具契约（blockMs / notifyOnOutput / requiredPermissions）、输出管道（头部内联 + 条件落盘）、后台任务生命周期与 turn 边界通知注入、沙盒优先执行模型（含"为什么有些命令沙盒里跑不了"的机理说明）、前端沙盒标签三态、E1–E5 分阶段路线。
...
- `docs/archive/v1/design-docs/execution-safety-agent-bash工具设计文档.md`（新增，现已归档）
