...
2026-09-09 用户根据输入框截图要求将“变更审查”名称改回 `Review`，并单独提交。同步输入框、右侧入口、对象菜单、页签及相关测试；中文说明和其余界面保持不变。此轮为小范围文案修订，不新增学习文档。

验证：Composer、流式消息和 Review 面板的聚焦回归通过；全工作区文档检查受到另一项未跟踪网站示例文档缺失 HTML 的影响，提交范围另行检查。
