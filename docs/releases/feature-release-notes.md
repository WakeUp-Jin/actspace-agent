...
| 日期 | 功能域 | 用户价值 | 变更摘要 |
| --- | --- | --- | --- |
| 2026-07-28 | DuckCoding 多模型接入 | 可以为 DuckCoding 配置多把 Key 和各自倍率，并让模型按需选择已有 Key；Codex 与 Grok 能按网关实际模型名、上下文和协议运行，单 Key 用户仍保持原有简单流程。 | 新增 DuckCoding 本地 Codex/Grok 模型档案、模型级凭据绑定与倍率计价；Codex 推理强度通过精确模型名切换并使用 Responses API、稳定缓存键和加密 reasoning 状态回放，Grok 与手动模型保留 Chat Completions；添加模型时可覆盖最大上下文。 |
| 2026-07-28 | Agent 等待反馈 | 模型首段响应较慢时不再留下空白消息区，用户能明确知道任务仍在运行；等待与工具执行使用统一、低干扰的动态视觉语言。 | 请求超过 300ms 且尚无正文、Thinking、工具、重试或审批状态时显示 `Operating Space · Expanding`；模型产生实际进展后立即隐藏，并将等待态与工具运行态统一为主题感知的黑灰 shimmer。 |
