...
| 事项 | 状态 | 入口 | 下一步 |
| --- | --- | --- | --- |
| 2026-05-27 使用问题拆分计划 | 进行中 | `docs/exec-plans/active/20260527-bugfix-foundation.md`、`docs/exec-plans/active/20260527-agent-tool-capabilities.md`、`docs/exec-plans/active/20260527-frontend-interaction-polish.md`、`docs/exec-plans/active/20260527-right-panel-views.md` | 按各 plan 的并行边界推进，避免同时改同一块 UI / tool contract。 |
...
| Tailwind 样式架构迁移 | 进行中 | `docs/exec-plans/active/actspace-tailwind-style-architecture.md` | Usage 样板已完成；继续迁移剩余主要前端区域，最后清理旧 CSS 并补 coding standards。 |
...
| Kairos 监控页产品化 | 进行中 | `docs/exec-plans/active/kairos-monitor-page-redesign.md` | 等待代码实现方案获批后，推进页面结构、执行列表、统计区和详情区验证。 |
...
- Kairos v1 七份基础计划已归档到 `docs/exec-plans/completed/kairos_*.md`；当前只保留监控页 redesign 作为 active。

## 未来方向

- Bash 全局执行策略选择器和真沙箱以 `docs/design-docs/agent-core/bash-policy-allowlist-design.md` 的 Phase 2 / Phase 3 为准，Phase 1 验收完成后再单独立项。
- Settings -> General -> Typography 已移入 `docs/exec-plans/tech-debt-tracker.md`，并由 `docs/exec-plans/active/20260527-frontend-interaction-polish.md` Task 5 承接。
