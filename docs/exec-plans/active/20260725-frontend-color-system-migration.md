...
1. `DESIGN.md`
2. `docs/design-docs/front-全局视觉语言规范.md`
3. `docs/design-docs/front-主题与配色规范.md`
4. `docs/design-docs/front-index.md`
5. 各组件专题规范
...
完整逐行分类见 `20260725-frontend-color-system-migration-audit.md`。该审计保留主仓库未提交工作区的 40 文件 / 159 行历史快照；用户选择以当前独立 worktree 一次性交付，因此实际迁移基线为 36 文件 / 147 行。主仓库额外 4 个 Settings 文件属于另一套未提交多供应商功能，没有跨功能复制；未来合入时会由 `check:frontend-theme` 阻止旧颜色消费者回流。
...
1. 新增 `docs/design-docs/public/front/ink-emerald-color-preview.html`。
...
7. 在 `docs/design-docs/front-index.md` 中增加样板入口和审批状态。
