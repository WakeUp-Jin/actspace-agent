...
在 5 个实现子计划完成后，统一完成工程验证、浏览器 mock 验证、Electron 真实验证、文档同步、history 记录和计划归档准备，确保前端交互与样式补齐任务可以被明确验收。
...
- Context popup 打开、关闭、entries 展示、浮层互斥。
...
- Context popup 在真实 session 中展示 contextState 或 snapshot。
