...
状态：Plan 0-5 与 Plan 7 完成；Plan 6 及 DuckDing 真实连接仍待用户统一手动验收
...
| [Plan 7](plan-7-duckding-multi-key.md) | 接入 DuckDing，并让模型仅在供应商存在额外 Key 时选择凭据 | provider credentials、manual model、runtime/UI binding | Plan 0-5 |
...
DeepSeek / Kimi / OpenRouter / DuckDing
```
...
- [x] Plan 7：DuckDing、额外 Key、公共模型目录、模型级 Key 选择与倍率定价已实现；真实 Key 调用待手动验收。
...
- 2026-07-27：DuckDing 沿用 OpenAI-compatible 协议；现有 provider Key 继续作为默认 Key，不迁移。额外 Key 只在供应商下管理，模型仅在存在额外 Key 时显示选择器，未绑定 `credentialId` 的模型继续继承默认 Key。
- 2026-07-27：Plan 7 实现完成：公共目录匿名读取 models.dev/OpenRouter 并缓存，模型保存元数据快照，runtime 按绑定 Key 选密钥并生成倍率后的价格快照；自动化覆盖单/多 Key、删除保护、目录离线和 Usage cache-write 计价。
