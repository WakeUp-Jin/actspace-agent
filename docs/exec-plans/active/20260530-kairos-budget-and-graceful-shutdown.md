...
- 相关文档：
  - `docs/design-docs/agent-core/kairos-autonomous-mode.md`（Kairos 自治事实来源；"非目标"曾写"不做配额护栏"，本期是 **cost budget** 演进，需更新；存储布局补 `memory/budget-state.json`）。
  - `docs/design-docs/agent-core/token-usage-and-context-state.md`（`llm_usage.cost` 成本事实来源）。
