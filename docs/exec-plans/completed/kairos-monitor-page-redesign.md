...
- 2026-05-28：`styles.css` 已按真实 Electron 窗口调整表格列宽和轨迹块宽度，避免中文类型列竖排、少量事件轨迹块过度拉伸。
...
- 2026-05-28：运行轨迹块改为 flex 小块并限制最大宽度。原因是事件数量少时 `1fr` grid 会把状态块拉成大色条，削弱“低密度运行轨迹”的语义。
