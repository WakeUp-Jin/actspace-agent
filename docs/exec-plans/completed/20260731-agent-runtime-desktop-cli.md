...
- `--data-dir` > `ACTSPACE_DATA_DIR` > 平台默认 `ActSpace` data root；CLI 与 Electron 使用同一个 canonical root。macOS 默认为 `~/Library/Application Support/ActSpace`。
- CLI v2 session root 固定为 `<dataRoot>/sessions-v2/`；本节中的旧 `<data-dir>/sessions/` 仅是 v1 历史实现记录。
