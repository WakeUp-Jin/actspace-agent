...
1. 在 `packages/shared/src/ipc.ts` 的 UsageActivitySnapshot schemaVersion 1 上追加可选 `aggregates`、`costSummary` 与查询 search 字段，保留旧调用方；在 `runtime-v2/desktop-ipc.ts`、`apps/desktop/src/global.d.ts`、preload 对应契约中透传。请求行追加可选原币金额/币种/来源；兼容 `costUsd` 只存 USD。
2. `costSummary` 定义 `amountsByCurrency`、`knownCostRequestCount`、`unknownCostRequestCount`、`unverifiedHistoricalRequestCount`；aggregates 为 provider/model/tool 分类完整匹配集。字段缺失表示旧协议，不等价于空结果。
...
4. 历史零值无可靠 provenance 时降级未知；已知缺陷版本显示对应原因，显式免费证据例外。旧非零保留原币金额，标记来源未验证。除原币已知金额外，不把旧 costUsd 转换结果当作新的实际费用事实。
