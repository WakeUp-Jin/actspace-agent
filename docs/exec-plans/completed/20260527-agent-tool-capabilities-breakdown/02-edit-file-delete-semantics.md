...
1. 先补失败用例
   - 删除唯一单行，确认不会留下多余空行。
   - 删除行内尾部文本，确认不会吞掉后续换行。
   - 删除内容本身以 diff marker 开头的行，确认 diff 统计不会漏算。
   - 删除唯一多行块，确认上下文行保留。
...
- [x] 补齐删除语义测试。
- [x] 修正测试暴露的 executor 问题。
- [x] 核对 preview / diff 统计。
- [x] 运行定向验证命令。
- [x] 更新必要文档和 history。
...
- 2026-06-02：删除换行只在 `old_string` 恰好从行首匹配且匹配后紧跟换行时发生；行内删除不得吞掉换行。diff 统计改为只数 unified diff hunk 内的真实 `+` / `-` 行，避免误算文件头或漏算 `---flag` / `+++flag` 这类内容行。
