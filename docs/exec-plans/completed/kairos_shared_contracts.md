...
- `AGENTS.md`
- `docs/design-docs/v1-legacy/agent-kairos-autonomous-mode.md` 的「契约定义」「事件存储与前端聚合」两章
- `packages/shared/src/session.ts`（理解现有 SessionEvent 形态、命名约定）
- `packages/shared/src/index.ts`（理解现有导出风格）

## 背景

- 相关设计文档：`docs/design-docs/v1-legacy/agent-kairos-autonomous-mode.md`
- 相关代码路径：
...
- [ ] Step 7：在 `docs/design-docs/v1-legacy/agent-kairos-autonomous-mode.md` 的「契约定义」一节顶部加一行链接指向本 plan，作为"已实现"标记；不改其它正文。
