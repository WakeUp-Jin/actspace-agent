...
> 历史子计划：未执行，随父计划于 2026-08-26 丢弃。文中的 v1 路径、`session.jsonl` 事件和 ToolScheduler 契约只用于追溯。
...
- main
  ...
  - `apps/desktop/src/main/user-config.ts`（新增）
- preload / renderer
  - `apps/desktop/src/preload/index.ts`
  - `apps/desktop/src/global.d.ts`
  - `apps/desktop/src/renderer/App.tsx`
  - `apps/desktop/src/renderer/components/messages/BashRunBlock.tsx`
  - `apps/desktop/src/renderer/fixtures/workbenchFixture.ts`
...
### 3. main 进程（apps/desktop/src/main）
...
`apps/desktop/src/preload/index.ts` 与 `apps/desktop/src/global.d.ts`：
