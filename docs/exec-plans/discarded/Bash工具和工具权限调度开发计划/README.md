...
生命周期：2026-08-26 丢弃。该计划基于 v1 `ToolScheduler`、`session.jsonl` 与 `packages/agent-core`，已被 v2 Tool Runtime ABI、Session Journal 和真实插件包结构取代。未来若实现动态 allowlist，必须从 [`../../../design-docs/execution-safety/README.md`](../../../design-docs/execution-safety/README.md) 新建立项。

这个文件夹保留 v1 Bash 工具、权限调度、审核 UI、暂停恢复与会话级 allowlist 的历史计划，不再作为当前执行入口。
...
| 5 | `actspace-bash-session-allowlist-plan.md` | 未执行，已丢弃 | 会话级动态 allowlist + Allow 子命令拆分授权（A+B）。历史设计见 `docs/archive/v1/design-docs/execution-safety-agent-bash-policy-allowlist-design.md`。 |
...
- `docs/archive/v1/design-docs/execution-safety-agent-bash-policy-allowlist-design.md`：历史 Bash 全局策略 + 动态 allowlist 设计，含 Phase 1/2/3 路线图。
