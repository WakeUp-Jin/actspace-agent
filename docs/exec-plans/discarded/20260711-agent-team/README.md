...
生命周期：2026-08-26 丢弃。该计划基于 v1 `SessionMeta`、`SessionEvent`、`ToolScheduler` 与 `packages/agent-core`，与当前 v2 one-shot Subagent、Session Journal 和领域插件包边界不兼容。Team 仍可作为未来产品方向，但必须从 `future-product-design` 重新设计并新建立项。
...
| [Plan 4](plan-4-desktop-integration.md) | 接入 session 创建、main registry、IPC、重启恢复和退出 flush | `apps/desktop/src/main/team/`、preload 契约 | Plan 0-3 |
