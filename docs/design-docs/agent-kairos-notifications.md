...
- **删除（2026-07-04 增补，方案对比见 `mockups/kairos-notification-delete-variants.html`）**：
...
`agent-kairos-autonomous-mode.md`（工具矩阵 / 事件流）、`agent-kairos-prompt-design.md`
（授权覆盖原则、机制段边界）、`front-Kairos监控页规范.md`（KairosPage 布局）。
