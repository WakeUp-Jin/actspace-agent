...
本文档是 Kairos 页面视觉与信息架构的前端事实来源。后端自治模型、存储、IPC 和聚合契约仍以 `docs/design-docs/v1-legacy/agent-kairos-autonomous-mode.md` 为准；本文只约束 renderer 的页面布局、组件关系和交互状态。
...
- 通知入口是详情面板直接可见的第 2 个 tab「通知」（挂未读徽标），不在头部单设按钮；右侧紧凑视图为「最终回复 ⇄ 通知」tab 切换。数据契约与后端设计见 `docs/design-docs/v1-legacy/agent-kairos-notifications.md`。
- `暂停` 的显示依据是运行态里的 `enabled === true`，不是单看 `state !== "stopped"`；后端停机后如果回推 `enabled: false`，主按钮要立刻切回 `开启`。
- `上下文` 按钮打开右侧滑入的 Sheet，展示当前 tick 会看到的系统提示词、会话历史与工具列表；详细规范见 `docs/design-docs/v1-legacy/front-Kairos监控页规范.md`。
...
- 默认选中 `最终回复`。
- `通知` tab 挂未读数徽标；内容是通知列表（点条目行内展开详情并标记已读），头部右侧为「全部已读」+「清理 ▾」下拉（清除已读 / 清空全部，后者原地二次确认）。条目 hover 时右上角浮现垃圾桶（单条删除，5 秒撤销窗口，列表底部出现「已删除 · 撤销」提示条）。详见 `docs/design-docs/v1-legacy/agent-kairos-notifications.md` §5。
...
`上下文` 按钮打开右侧滑入 Sheet，展示 Kairos 当前 tick 会看到的系统提示词、短期记忆和工具列表。Kairos 自治模型、prompt 组装和短期记忆长期事实仍见 `docs/design-docs/v1-legacy/agent-kairos-autonomous-mode.md`；本节只约束 renderer 入口、Sheet 行为、Snapshot 契约和验收点。
