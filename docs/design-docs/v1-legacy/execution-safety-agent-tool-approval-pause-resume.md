...
> 归档说明：本文描述 v1 ApprovalGate、PendingApprovalRegistry 与旧 Session 恢复方案。当前 v2 以 Tool Runtime ABI、`@actspace/tools-approval` 和 Host ApprovalBroker 为准。
...
| 层 | 位置 | 职责 |
|---|---|---|
| ApprovalGate | v2 Runtime Tool Runtime | 接口契约。Runtime 只知道“调用它就能等到结果”。 |
...
- 不让 ToolScheduler 直接依赖 Electron IPC（保持 v2 Runtime 无 Electron 依赖）。
- 不在 renderer 中执行工具或持有 resolve 函数（安全边界）。
- 首版不做磁盘持久化恢复执行（安全优先）。

## 相关文件

- `packages/agent-runtime/src/tools/scheduler.ts` — ApprovalGate 接口和 ToolScheduler 改造
- `apps/desktop/src/main/approval-registry.ts` — PendingApprovalRegistry 实现
- `packages/agent-runtime/src/engine/types.ts` — AgentEvent 扩展
- `packages/shared/src/session.ts` — RuntimeStreamEvent 扩展
- `packages/shared/src/ipc.ts` — IPC 类型
- `apps/desktop/src/main/index.ts` — IPC handler 和 registry 单例
- `apps/desktop/src/preload/index.ts` — renderer API 暴露
- `packages/agent-runtime/src/engine/bridge.ts` — 事件映射
- `packages/agent-runtime/src/engine/create-agent-deps.ts` — 配置链路
- `apps/desktop/src/main/agent-run.ts` — 接通 registry
