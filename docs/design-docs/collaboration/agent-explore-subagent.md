...
本文是 Explore 子代理的长期设计事实来源。通用子代理运行时（隔离上下文、transcript、落盘、abort 级联）仍以 [`docs/design-docs/collaboration/agent-subagent-runtime.md`](agent-subagent-runtime.md) 为准，Explore 复用其中绝大部分运行时机制，只在**模型、scope、展示**三点上分叉。
