...
提示词的**缓存形态**约束（静态前缀 + 动态尾部）见 `agent-kairos-prompt-cache-optimization.md`，本文不重复。
...
- 本仓库：`agent-kairos-autonomous-mode.md`（Kairos 总体设计）、
  `agent-kairos-prompt-cache-optimization.md`（缓存形态约束）、`front-设置页规范.md`（设置页交互约定）。
