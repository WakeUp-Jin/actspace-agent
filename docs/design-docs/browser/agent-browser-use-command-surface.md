...
这 62 条已经全部进入 Go registry 并标记为 `implemented`，但不等于向模型平铺 62 个 tools。模型通过 9 个分类工具、`browser_help` 和 `browser_run` 使用它们。当前架构入口见 `docs/design-docs/browser/agent-browser-use-index.md`；真实 Chrome 验收状态见 Plan 5。
