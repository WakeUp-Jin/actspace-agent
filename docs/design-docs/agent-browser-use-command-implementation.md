...
本文档从 CDP 原语出发，定义 actspace-agent 全部 62 条浏览器命令的核心实现逻辑。Plan 5 已完成 62/62 Go canonical handlers、轻量 Locator runtime、事件状态、Agent 分类工具与真实 Chrome profile 验收。当前架构入口见 `agent-browser-use-index.md`。
...
- `agent-browser-use-command-surface.md`：定义命令的**参数契约和行为语义**（是什么）。
- `agent-browser-use-integration-design.md`：定义**架构分层和工具暴露**（怎么集成）。
- `agent-browser-bridge-design.md`：定义**三层职责边界和 backend 分工**（架构边界）。
