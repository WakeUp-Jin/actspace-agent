...
- `search_files`：搜索 workspace 内文本或文件名，优先用快速本地搜索能力。
- `list_directory`：列出目录，支持最小文件导航。
- `edit_file_diff`：生成 diff preview，不自动写盘。
...
- 工具错误要精确，避免污染下一轮模型判断。
- `edit_file_diff` 默认只产物化 diff，不应用 patch。
...
- 增加 mock fixtures，覆盖 thinking、read/search、edit diff、context snapshot、error。
...
- 实现 `read_file`、`search_files`、`list_directory`、`edit_file_diff`。
- 增加路径边界、输入校验和结构化错误。
...
- 完整 turn 至少包含 `thinking + read_file + search_files 或 list_directory + edit_file_diff + assistant_message + context_snapshot`。
