...
- 按钮尺寸对齐 Cursor 风格：高 32px（`h-8`）、圆角 7px、13px 字号、`px-3` 水平内边距；`⋯ 更多` 为 32×32 方形图标按钮。header 纵向内边距 `py-3`。
- 通知入口是详情面板直接可见的第 2 个 tab「通知」（挂未读徽标），不在头部单设按钮；右侧紧凑视图为「最终回复 ⇄ 通知」tab 切换。数据契约与后端设计见 `agent-kairos-notifications.md`。
...
- 顶部使用 Cursor 风格 segmented control（浅灰槽 `bg-surface-subtle` + 激活项白底微浮起，高度 28px），直接可见的只有 `最终回复` / `通知` / `更多 ▾` 三格（2026-07-04 起）；`工具结果` 与 `思考过程` 收进 `更多` 下拉菜单，选中后 `更多` 格显示当前标签名并高亮为激活态。tab 弹性收缩铺满一行，不允许把面板撑出水平滚动条。
- 面板内部是三行 grid（tab 栏 / 标题行 / 内容区），只有内容区滚动；滚动长内容时 tab 栏和标题行保持固定，不允许被卷走。
- 默认选中 `最终回复`。
- `通知` tab 挂未读数徽标；内容是通知列表（点条目行内展开详情并标记已读），头部右侧为「全部已读」+「清理 ▾」下拉（清除已读 / 清空全部，后者原地二次确认）。条目 hover 时右上角浮现垃圾桶（单条删除，5 秒撤销窗口，列表底部出现「已删除 · 撤销」提示条）。详见 `agent-kairos-notifications.md` §5。
