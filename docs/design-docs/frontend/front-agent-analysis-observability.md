...
- 在设置导航中新增真实分区「分析观测」，位置在「归档会话」之后、「更新」之前，并使用与其他设置分区一致的选中态。
- 点击后保留设置导航，只在右侧设置内容区展示会话索引，不提前把 `WorkbenchLayout.view` 切换为独立 `analysis` 视图。
- 会话索引列出全部未归档 Session，包括暂无 Trace 的会话；当前活动 Session 只标记为「当前」，不自动钻取。
- 用户显式选择 Session 后，才把 `WorkbenchLayout.view` 切换为独立 `analysis` 详情工作区并加载该 Session。
- Session 索引与详情是两个页面状态，不在详情页常驻为第三栏。详情仍只保留 Agent Run / Turn 导航与 LLM Call 内容两栏。
- 从详情返回时恢复「分析观测」设置分区，并保留索引的搜索、状态筛选、模型筛选、缓存数据和滚动位置。

### 单会话独立工作区

- 仅在用户选择具体 Session 后隐藏设置导航和聊天侧栏；单会话详情占据完整工作区。
- 设置内会话索引不提供自己的返回箭头；返回应用继续由左侧设置导航负责。
- 会话详情左上角提供无边框纯箭头，Accessible Name 与 Tooltip 使用「返回分析观测」，返回设置内索引。
- 设置内索引为单列表，独立详情为两栏。
...
- 索引通过单次 main IPC 返回聚合后的轻量列表，Renderer 不逐条读取完整 Trace。
- 索引在设置右侧内容区使用约 `1180px` 的最大阅读宽度，不把少量会话横向铺满整个窗口。
...
- 设置内索引不重复提供返回动作；详情复用设置导航的无边框返回语法，不使用独立描边方形图标按钮。
- 设置内索引标题使用「分析观测」与会话数量，导航路径由左侧设置导航承担，不重复显示「返回」或「本地记录」文字。
...
- 从设置进入时在右侧内容区显示会话索引并保留设置导航；当前会话仅标记，不自动读取详情。
- 索引只读取 Session 元数据与 summary sidecar；单条损坏不阻断其他会话。
- 从索引选择会话后才进入独立详情；返回后仍位于「分析观测」分区，并恢复搜索、筛选与滚动位置。
...
- Settings 侧栏在会话索引中保留、在单会话详情中消失；页面始终最多两栏。
...
- 2026-07-30：页面采用两栏，不增加 Session 导航；当时 Settings 入口进入后整页接管，该入口行为已由 2026-08-02 决策替代。
- 2026-07-30：用户输入作为 Agent Run 分组，Turn 在左栏，LLM Call 在右栏小型切换器中处理。
- 2026-08-02：会话索引改为设置右侧真实分区；只有选择 Session 后才升级为独立详情工作区，返回时恢复索引状态。
