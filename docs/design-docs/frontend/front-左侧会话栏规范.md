...
- 状态指示：当前选中与运行状态分离。选中只使用中性灰底和文字层级；有运行中 turn / 待审批工具时才显示行首 operational 绿点。
- 全局设置入口：底部 Settings。
...
- `Usage`：统计页占位（Coming soon），将承载 token / 成本聚合。具体设计见 `docs/design-docs/frontend/front-usage-statistics.md`。
...
- marker 列宽 14px，内含两层（同位绝对叠加，靠 opacity 互斥切换，避免 layout 抖动）：
  - **`<span class="session-status-dot">`** — 状态点：
    ...
    - `is-active`：不再用彩色点表达选中；选中由中性 `selected` 背景、主文字和必要的字重变化承担。
    - `is-busy`：该会话有运行中 turn / 待审批工具时，使用 operational 绿色小点；允许极轻呼吸动画，并尊重 `prefers-reduced-motion`。
...
- Sidebar surface 比主工作区深一层，但仍属于暖中性灰阶。
- hover、selected、pressed 使用连续灰阶，不使用蓝色点、蓝色图标或绿色整行背景。
- operational 绿色只用于 busy / running / connected 等真实运行状态。
- Pinned、Archive、新建等普通图标保持 `currentColor` 中性灰黑。
- 状态必须同时具备 tooltip、文字或可访问标签，不能只依赖绿点。
