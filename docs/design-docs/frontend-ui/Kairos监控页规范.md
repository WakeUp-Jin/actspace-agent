...
桌面端主区域使用两列布局：左侧执行列表约 45%，右侧详情列约 55%。右侧再上下分为统计区和详情区。`xl` 以下可以退化为单列：轨迹在上，执行列表在中，统计和详情在下。
...
运行轨迹只表达最近一段时间的事件密度和关键状态，不承担完整日志职责。
...
轨迹高度应比早期宽表版本更低，目标是一个紧凑 band：标题 + 小 legend + 低矮块状 timeline。hover 可显示时间、类型、摘要；点击轨迹块时可以同步选中左侧对应执行列表行。
