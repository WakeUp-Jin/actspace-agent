...
- `主题与配色规范.md`：三态主题（浅/深/跟随系统）机制与「颜色必须随主题翻转」的硬约束；任何写颜色的样式工作先读这里。
...
- `Kairos上下文Sheet规范.md`：Kairos 监控页"上下文"按钮与右侧滑入 Sheet 的入口、信息架构、Snapshot 契约与 IPC 通道。
...
- `消息可视化转换规范.md`：在回复操作条加"可视化"按钮，用主模型把 Markdown 回复转 HTML 并在右侧渲染；含缓存（生成一次、持久化、后续读缓存）与安全沙箱约束、V1/V2 边界。
