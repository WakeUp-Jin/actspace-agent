...
- `docs/archive/v1/design-docs/execution-safety-agent-bash-policy-allowlist-design.md`
...
- `docs/archive/v1/design-docs/execution-safety-agent-bash-policy-allowlist-design.md:156-165` 明确 Phase 1 决定放开 `|` 进入授权拆分；当前 `packages/agent-core/src/tools/tools/bash/permissions.ts:17-18` 的 `UNSUPPORTED_SHELL_SYNTAX_RE = /[|<>`$(){}]/` 仍包含管道，`packages/agent-core/src/tools/tools/bash/permissions.ts:113-124` 会在权限检查早期直接 deny。
