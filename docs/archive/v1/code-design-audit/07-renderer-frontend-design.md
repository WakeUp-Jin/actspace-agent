...
- 偏移点：`docs/archive/v1/design-docs/lab-README.md` 明确“V0 renderer mock 已落地；后端 Lab Runtime、IPC 和持久化尚未实现”；实现上 `apps/desktop/src/renderer/components/LabPage.tsx:242-253` 完全用本地 `useState` 管理 cards、completedExperiments、dialog、newTitle 等状态，`281-291` 创建实验也只写本地状态，没有 IPC 或持久化。
