...
- `apps/desktop/src/main/kairos-bootstrap.ts`
- `apps/desktop/src/main/kairos-ipc.ts`
- `apps/desktop/src/main/kairos-ipc-internals.ts`
- `apps/desktop/src/main/test/kairos-bootstrap.test.ts`
- `apps/desktop/src/main/test/kairos-ipc-internals.test.ts`
- `apps/desktop/src/renderer/pages/KairosPage.tsx`
- `apps/desktop/src/renderer/state/useKairos.ts`
- `apps/desktop/src/renderer/components/right-panel/KairosRightPanelView.tsx`
...
- 偏移点：长期设计文档后半段把 `blocklist.timeWindows` 描述为 scheduler 硬拦截、把 `tickBudget.perHour` 描述为超限后自动 stop 并 emit error（`docs/archive/v1/design-docs/agent-kairos-autonomous-mode.md:897`, `docs/archive/v1/design-docs/agent-kairos-autonomous-mode.md:1322`, `docs/archive/v1/design-docs/agent-kairos-autonomous-mode.md:1323`），但 `QueueProcessor` 文件头仍明确写着“不实现 blocklist.timeWindows 推迟 / tickBudget 限额”（`packages/agent-core/src/kairos/scheduler.ts:10`, `packages/agent-core/src/kairos/scheduler.ts:11`, `packages/agent-core/src/kairos/scheduler.ts:12`）。
...
- 可读性问题：设计文档同一文件前段也保留了“v1 不硬执行”的历史说明（`docs/archive/v1/design-docs/agent-kairos-autonomous-mode.md:16`），后段又把它写成验收项，读者很难判断当前真相源。
...
- 偏移点：审查计划和设计目标要求“ring buffer 不够再读 jsonl”，IPC 契约也保留 `before?: EventId`（`packages/shared/src/kairos-contracts.ts:143` 到 `packages/shared/src/kairos-contracts.ts:153`）；但 main handler 忽略 `before`，只返回 `controller.getRecentEvents(limit)`，并固定 `hasMore: false`（`apps/desktop/src/main/kairos-ipc.ts:73` 到 `apps/desktop/src/main/kairos-ipc.ts:78`）。
...
- 死代码/兼容残留：`before` 字段目前是未接线的契约残留；文档中也同时存在“应该回填”和“v1 不回填”的冲突说明（`docs/archive/v1/design-docs/agent-kairos-autonomous-mode.md:40`, `docs/archive/v1/design-docs/agent-kairos-autonomous-mode.md:380`）。
...
- 死代码/兼容残留：`docs/archive/v1/design-docs/agent-kairos-autonomous-mode.md:178` 仍列出 `KairosEventDetail` 可展示 thinking，但当前默认页面只有最终回复/工具结果两个 tab（`apps/desktop/src/renderer/pages/KairosPage.tsx:725` 到 `apps/desktop/src/renderer/pages/KairosPage.tsx:769`）。
...
- 偏移点：设计把 desktop IPC 定位为薄控制/转发层；但 `registerKairosIpc()` 注册匿名 listener 到 controller（`apps/desktop/src/main/kairos-ipc.ts:154`, `apps/desktop/src/main/kairos-ipc.ts:155`），`dispose()` 只 dispose batcher 和 remove IPC handler，明确不 off controller listener（`apps/desktop/src/main/kairos-ipc.ts:157` 到 `apps/desktop/src/main/kairos-ipc.ts:166`）。
- 不合理设计：settings 更新会 stop 旧 controller、dispose IPC、再重建 controller 和 IPC（`apps/desktop/src/main/index.ts:689` 到 `apps/desktop/src/main/index.ts:701`）。旧 controller 上残留的 listener 捕获已 disposed 的 batcher；虽然 batcher 会 no-op，但 listener 生命周期不再可追踪。
- 可读性问题：注释说“dispose 后 batcher 不会再产生副作用，因此显式 off 已经无意义”（`apps/desktop/src/main/kairos-ipc.ts:160` 到 `apps/desktop/src/main/kairos-ipc.ts:162`），这解释了无副作用，但没有解释为什么可以接受 listener 泄漏。
