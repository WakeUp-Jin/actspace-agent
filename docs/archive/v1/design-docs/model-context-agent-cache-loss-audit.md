...
> 归档说明：本文是 v1 Agent Core 的 Cache Audit sidecar 设计。当前 v2 只以 Session Journal 中的 `request/snapshot` 与 `llm/usage` 作为请求和缓存观测事实，不实现本文的独立 audit 目录。

> 状态：设计规范
> 关联计划：`docs/exec-plans/completed/actspace-cache-loss-audit-plan.md`
> 适用范围：历史 v1 Agent Core、`packages/shared`、本地 session 存储与排障脚本；v2 迁移以 `agent-plugin-runtime/` Session / LLM 契约为准。
...
- v2 Runtime 的 LLM usage event builder
...
- `docs/archive/v1/design-docs/model-context-context-compression.md` 已说明历史压缩会影响 DeepSeek prompt cache，本设计提供压缩前后证据。
