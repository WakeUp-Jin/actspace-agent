...
本文档是 Kairos 页面视觉与信息架构的前端事实来源。后端自治模型、存储、IPC 和聚合契约仍以 `docs/archive/v1/design-docs/agent-kairos-autonomous-mode.md` 为准；本文只约束 renderer 的页面布局、组件关系和交互状态。

Kairos 默认不作为工作台能力展示。只有用户在「设置 > Kairos」开启功能后，左侧栏、右侧对象启动页与 `+` 菜单才显示入口；关闭功能时已打开的 Kairos 右侧 Tab 会被移除，全屏 Kairos 路由退回聊天页。这里的功能开关只控制入口与运行时是否可用，页面内的「开启 / 暂停」仍单独控制自主循环。
...
- 右侧按钮组直出 `开启 / 暂停`、`唤醒` 两个高频按钮；低频操作 `上下文`、`重置` 收进末尾的 `⋯ 更多` 下拉（2026-07-04 调整，避免按钮过多）。未启用时首按钮变为 `开启`。
- 按钮保持编辑器密度：高 32px（`h-8`）、圆角约 7px、13px 字号、`px-3` 水平内边距；`⋯ 更多` 为 32×32 方形图标按钮。header 纵向内边距 `py-3`。
- 通知入口是详情面板直接可见的第 2 个 tab「通知」（挂未读徽标），不在头部单设按钮；右侧紧凑视图为「最终回复 ⇄ 通知」tab 切换。数据契约与后端设计见 `docs/archive/v1/design-docs/agent-kairos-notifications.md`。
- `暂停` 的显示依据是运行态里的 `enabled === true`，不是单看 `state !== "stopped"`；后端停机后如果回推 `enabled: false`，主按钮要立刻切回 `开启`。
- `上下文` 按钮打开右侧滑入的 Sheet，展示当前 tick 会看到的系统提示词、会话历史与工具列表；详细规范见 `docs/archive/v1/design-docs/front-Kairos监控页规范.md`。
...
| 事件 | 颜色 | 说明 |
|---|---|---|
| `reply` / 最终回复 | 低饱和 info 蓝或中性墨色 | 只用于区分内容类型，不表示运行状态 |
| `running` / 健康运行 | 翡翠绿 | 只用于正在运行、在线与健康状态 |
| `sleep` / waiting | 中性灰 | 睡眠和等待不使用 warning 色 |
| `warning` / 待关注 | 琥珀色 | 风险、额度或需要注意的状态 |
...
- 状态 badge 可以使用颜色：`running` / `healthy` 使用 operational green，`failed` 使用 danger，`warning` 使用琥珀；普通 `success` 可以用弱绿色文字或状态点，但不把整行染绿。
- `最终回复` 和 `工具执行` 行必须可点击，因为右侧详情区会展示对应内容。
- 选中行使用中性 `selected` 背景和主文字，不使用蓝色或绿色大面积底色。
...
- 顶部使用中性 segmented control（`bg-surface-subtle` 槽 + 激活项 surface 与细边界，高度 28px），直接可见的只有 `最终回复` / `通知` / `更多 ▾` 三格（2026-07-04 起）；`工具结果` 与 `思考过程` 收进 `更多` 下拉菜单，选中后 `更多` 格显示当前标签名并高亮为激活态。tab 弹性收缩铺满一行，不允许把面板撑出水平滚动条。
- 面板内部是三行 grid（tab 栏 / 标题行 / 内容区），只有内容区滚动；滚动长内容时 tab 栏和标题行保持固定，不允许被卷走。
- 默认选中 `最终回复`。
- `通知` tab 挂未读数徽标；内容是通知列表（点条目行内展开详情并标记已读），头部右侧为「全部已读」+「清理 ▾」下拉（清除已读 / 清空全部，后者原地二次确认）。条目 hover 时右上角浮现垃圾桶（单条删除，5 秒撤销窗口，列表底部出现「已删除 · 撤销」提示条）。详见 `docs/archive/v1/design-docs/agent-kairos-notifications.md` §5。
- `最终回复` tab 必须完整显示最近或选中的最终回复正文，例如：`环境无变化，仍无配置路径、无会话、无 briefs。继续休眠。`
- 选中执行列表里的 `reply` 行时，详情区仍停留在 `最终回复` tab 并显示该回复。
- 选中执行列表里的 `tool` 行时，可以自动切到 `工具结果` tab（此时 `更多` 格直接显示「工具结果」），展示 tool name、输入摘要、输出摘要和错误信息。长工具结果允许折叠或截断。
...
`上下文` 按钮打开右侧滑入 Sheet，展示 Kairos 当前 tick 会看到的系统提示词、短期记忆和工具列表。Kairos 自治模型、prompt 组装和短期记忆长期事实仍见 `docs/archive/v1/design-docs/agent-kairos-autonomous-mode.md`；本节只约束 renderer 入口、Sheet 行为、Snapshot 契约和验收点。

### 入口与行为

- 入口位于 Header 右侧 `⋯ 更多` 下拉内（2026-07-04 起与 `重置` 一同收纳）。
- `上下文` 是只读查看，排在 `重置` 前，符合先查看、后破坏的心智。
...
- 运行轨迹颜色使用绿、琥珀、红、灰四类状态色，并允许最终回复使用一档低饱和 info 蓝；颜色只编码类型和状态，不承担选中层级。
