...
> **历史基线提示**：本文记录重构前的独立 Usage 仪表盘方案。当前 Usage 产品定位、信息架构和数据边界以 [`front-设置中心重构规范.md`](../../../design-docs/frontend/front-设置中心重构规范.md) 为准；本文保留用于迁移追溯。

> 2026-09-07 已实施的当前页面见[设置内使用统计页面更新](../../../design-docs/frontend/front-usage-statistics-refresh.md)；真实宿主验收边界见该任务摘要；其中的目录与费用方案不由本文的旧仪表盘口径推导。
...
- 字体：复用全局 `--font-ui`（macOS 系统字体优先），**不在 Usage 单独引入 Plus Jakarta Sans 等装饰字体**，以免和聊天态、设置态字形不一致。详细字体栈见 [全局视觉语言规范](../../../design-docs/frontend/front-全局视觉语言规范.md) 的「字体栈」段。
...
- **`"global"`（默认）**：扫描所有事实源后合并聚合，得到"账单全貌"。这是页面当前唯一对用户开放的范围。
  - 普通对话：v2 Session Journal 中全部已发布的事件事实。
  - 其他 v1 产品事件不再作为 Usage 的隐式事实源；若需要迁移，必须先定义显式 v2 codec 和导入边界。
...
- **会话明细**（底部表格专用）：在聚合器内维护 `Map<sessionId:turnId → requestRow>`，把同一 turn 内的多次 `llm_usage` 累加为一行；`timestamp` 取该 turn 内最新 usage 时间；`model` 取 token 最高的模型；`modelCallCount` 记录折叠的调用次数；`workspaceId/workspaceRoot` 来自 v2 Session meta；排序完成后按每页 10 条切片返回。
- 工具调用：`tool_call.payload.name`；`tool_result.ok = false` 累加 failedCount。
- 日期活跃度：按事件 `timestamp.slice(0, 10)` 的日期聚合。

### 数据边界

- 事实来源为 v2 Session Journal；旧 `session.jsonl` 与 Kairos 短期记忆段不再由当前 Usage 页面直接读取。`context-state.json` 不作为统计页输入。
...
- v1 产品的独立累计文件不进入 Usage 页面账本；Usage 总是按 v2 Session Journal 的事件级真相重建。
