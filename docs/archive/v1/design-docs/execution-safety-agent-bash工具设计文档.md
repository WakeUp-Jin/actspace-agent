...
> 归档说明：本文保留 v1 Bash executor、输出和沙盒实现经验。当前 v2 的 definition、policy、approval、prepared execution、Host port 与 result 契约以 Tool Runtime ABI 和 `packages/tools/` 为准。

## 当前状态

> 本文保留 v1 Bash 行为和沙盒实现经验；v2 的 Tool Runtime / Host capability 边界以 `docs/design-docs/agent-plugin-runtime/agent-spec-tool-runtime-abi.md` 为准。
...
- `docs/archive/v1/design-docs/execution-safety-agent-bash-policy-allowlist-design.md`：权限层设计（allowlist、审批 UI、执行策略）。分工：该文管"命令能不能跑、要不要问用户"，本文管"命令怎么跑、输出怎么回、跑不完怎么办、在什么约束下跑"。
...
2. **loop 已结束**（agent 已回复完毕、处于空闲）：通知滞留队列，用户下一次发消息时随该轮注入。v2 不提供后台 autonomous tick 唤醒。
...
profile 中包含运行时才确定的路径（workspace root、会话 tmp、用户 home 下敏感路径），因此**版本化进仓库的是生成器代码**（未来位于 `packages/agent-runtime/src/tools/tools/bash/sandbox/`），profile 实例每会话生成到 session tmp（如 `<userData>/tmp/<sessionId>/sandbox.sb`），随会话清理。Cursor 与 Claude Code 均为动态生成。
...
2. **Seatbelt 无法按域名过滤网络**（过滤器只认 IP/端口）。域名白名单的真实架构：profile 全拒网络、只放行本地代理端口 → Host 起本地代理做域名过滤 → 子进程注入 `HTTP_PROXY`/`HTTPS_PROXY`。这是 Cursor / Claude Code 的共同做法，也是自研路线的最大增量（代理本身 + 不认代理环境变量的工具的兜底）。
...
- E5 需额外写清：沙盒 profile 的具体 allow/deny 清单、违规标注的检测方式、升级审批与 `docs/archive/v1/design-docs/execution-safety-agent-bash-policy-allowlist-design.md` 审批流的合并方案。
