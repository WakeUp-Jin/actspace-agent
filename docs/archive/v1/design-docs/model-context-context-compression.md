...
- 不做用户手动增删改上下文条目（属于 `docs/design-docs/model-context/agent-token-usage-and-context-state.md` 的后续）。
...
安全权衡（必须在实现时同步更新 `docs/SECURITY.md` 与 `docs/design-docs/execution-safety/agent-权限设计规则和原则.md`）：
...
- `docs/design-docs/model-context/agent-token-usage-and-context-state.md`：token 估算、`context_snapshot`、`context-state.json` 的数据分层。
- `core-storage-and-observability.md`：`<userData>/tmp`、`session.jsonl`、run-log 边界。
- `docs/archive/v1/design-docs/agent-kairos-autonomous-mode.md` + `kairos/compression/`：Kairos 短期记忆压缩，prompt 风格可借鉴但与主 Agent 解耦。
- `docs/design-docs/tool-system/agent-tool-preview-design-guidelines.md`：`previewKind` 与 `ToolUiPreview` 契约（分型依据）。
- `docs/design-docs/execution-safety/agent-权限设计规则和原则.md` / `docs/SECURITY.md`：读边界放开需同步更新。
