...
- 全局 CSS 只保留 token、base、滚动条、focus 和复杂第三方渲染边界。
...
旧 `styles.css` 一旦继续增长，很快会出现两个问题：样式位置离 DOM 太远，改一个页面要在大文件里找上下文；类名前缀看似隔离，但状态、响应式和 hover 仍然分散。Tailwind utility 把这些视觉决策放回组件附近，适合页面仍在快速演进时使用。
