...
| 可观测性 | B+ | Session V2 已统一 `agentRunId → turnId → llmCallId`；分析观测先以 Session 元数据和 summary sidecar 建立跨会话索引，再按 Session / Agent Run 懒加载脱敏请求上下文、响应和差异，并具备 64 MiB 单 Run 上限、30 天 / 512 MiB retention、损坏隔离与显式清理。 | 完成真实 Electron 长会话、Retina 和多 provider 人工验收；若要对齐网络代理级观测，再补原始 HTTP wire request/stream 的安全采集。 |
| 安全 | B | Electron 边界采用 `contextIsolation` + typed preload；Trace 在 main 侧校验路径与身份并执行脱敏、有界读取；Terminal renderer 不能传 executable、cwd 或 env；provider Key 使用 main-only `0600` 明文文件，并具备旧密文全量迁移和损坏时禁止覆盖，但不再具备静态加密保护。 | 补 API Key 替换/轮换、凭据文件诊断/恢复入口、外部 URL 窄化打开桥和敏感路径按需读审核。 |
| 学习沉淀 | B | 体系已经跑起来，且已有学习文档。 | 随着 DeepSeek 接入和前端对接，继续补真正有迁移价值的学习文档。 |
| Kairos 自治模式 | B+ | v1 闭环已落地，KairosPage 已从原始事件表改成监控台式两列 UI；产品功能现默认隐藏且不创建 Controller，只能从设置显式开放，并与自主循环、额度护栏分层；上下文已重构为「静态前缀 + 动态尾部」，thinking 全链路落盘/重放/展示，短期记忆压缩已接线。 | 真实 tick 缓存命中率（目标 ≥85%）和功能门控 Electron 交互待手动验收；intra-day 压缩与压缩 LLM 调用的用量计费未做；先继续优化核心效果，再考虑提升默认可见性。 |
