...
在 actspace-agent 设计「上下文压缩」（`docs/design-docs/v1-legacy/model-context-context-compression.md`）时，针对 bash 工具的大输出处理收敛出一套明显更优的设计。回头看 `.agents/skills/llm-agent-dev` 对 bash 的指导，**输出处理这一块太粗糙**，会把使用者带进「内存被大输出吃光」的坑。本文件沉淀该设计，作为后续统一修复 skill 源文件的依据。
...
事实来源：`docs/design-docs/v1-legacy/model-context-context-compression.md` 的「预防层 A：bash 流式落盘」。
...
- 2026-05-29：bash 输出处理定为「流式落盘 + 头部截断 + 文件指针」，不走 LLM 摘要；摘要只服务 web/generic 等重跑恢复型工具。原因见 `docs/design-docs/v1-legacy/model-context-context-compression.md`。
