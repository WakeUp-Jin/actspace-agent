...
function snapshot(search = "", status = "all", page = 1): UsageActivitySnapshot {
  ...
  const costSummary = { amountsByCurrency: models.length ? { USD: models.reduce((sum, r) => sum + (r.costAmount ?? 0), 0) } : {}, knownCostRequestCount: models.filter((r) => r.costAmount != null).length, unknownCostRequestCount: models.filter((r) => r.costAmount == null).length, unverifiedHistoricalRequestCount: 0 };
