          ...
          <>
            <div className="flex min-h-0 min-w-0 flex-1">
              {!workspaceRoot && !sessionId ? <Centered icon={<AlertTriangle size={18} />} title="选择工作区" message="请先在输入框中选择工作区，再打开变更审查。" />
                ...
                : state.error ? <Centered icon={<AlertTriangle size={18} />} title="变更审查不可用" message={state.error} action={<button type="button" className="mx-auto inline-flex h-8 items-center gap-1.5 rounded-act-sm bg-action px-3 text-[12px] font-medium text-on-action" onClick={refresh}><RefreshCw size={13} />刷新</button>} />
...
function GitDialog({ workspaceRoot, onClose, onDone }: { workspaceRoot?: string; onClose: () => void; onDone: () => void }) {
  ...
  return <DialogShell title="提交或推送" onClose={onClose}><div className="grid gap-3 p-4"><label className="grid gap-1 text-[12px] text-text-muted">提交说明<input autoFocus className="h-9 rounded-act-sm border border-line bg-surface px-2 text-[13px] text-text-main outline-none focus:border-focus-ring" value={message} onChange={(event) => setMessage(event.target.value)} /></label>{error ? <div className="text-[12px] text-danger">{error}</div> : null}<div className="flex justify-end gap-2"><button type="button" className="h-8 px-3 text-[12px] text-text-muted" onClick={onClose}>取消</button><button type="button" disabled={busy || !message.trim()} className="h-8 rounded-act-sm border border-line px-3 text-[12px] text-text-main disabled:opacity-40" onClick={() => void run(false)}>提交</button><button type="button" disabled={busy || !message.trim()} className="inline-flex h-8 items-center gap-1.5 rounded-act-sm bg-action px-3 text-[12px] font-medium text-on-action disabled:opacity-40" onClick={() => void run(true)}>{busy ? <Loader2 size={13} className="animate-spin" /> : null}提交并推送</button></div></div></DialogShell>;
}
...
function PullRequestDialog({ workspaceRoot, onClose }: { workspaceRoot?: string; onClose: () => void }) {
  ...
  return <DialogShell title="创建拉取请求" onClose={onClose}><div className="grid gap-3 p-4"><label className="grid gap-1 text-[12px] text-text-muted">标题<input autoFocus className="h-9 rounded-act-sm border border-line bg-surface px-2 text-[13px] text-text-main outline-none focus:border-focus-ring" value={title} onChange={(event) => setTitle(event.target.value)} /></label><label className="grid gap-1 text-[12px] text-text-muted">目标分支<input className="h-9 rounded-act-sm border border-line bg-surface px-2 text-[13px] text-text-main outline-none focus:border-focus-ring" value={baseBranch} onChange={(event) => setBaseBranch(event.target.value)} /></label><label className="grid gap-1 text-[12px] text-text-muted">说明<textarea className="min-h-24 resize-y rounded-act-sm border border-line bg-surface p-2 text-[12px] text-text-main outline-none focus:border-focus-ring" value={body} onChange={(event) => setBody(event.target.value)} /></label><label className="flex items-center gap-2 text-[12px] text-text-main"><input type="checkbox" checked={draft} onChange={(event) => setDraft(event.target.checked)} />创建为草稿</label>{message ? <div className="text-[12px] text-text-muted">{message}</div> : null}<div className="flex justify-end gap-2"><button type="button" className="h-8 px-3 text-[12px] text-text-muted" onClick={onClose}>取消</button><button type="button" disabled={busy || !title.trim()} className="inline-flex h-8 items-center gap-1.5 rounded-act-sm bg-action px-3 text-[12px] font-medium text-on-action disabled:opacity-40" onClick={() => void create()}>{busy ? <Loader2 size={13} className="animate-spin" /> : <GitPullRequestArrow size={13} />}创建 PR</button></div></div></DialogShell>;
}
