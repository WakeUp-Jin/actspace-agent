  ...
  return (
    <aside className="flex h-full min-h-0 w-full flex-col bg-surface-raised" aria-label="已变更文件">
      <div className="flex items-center gap-1 border-b border-line p-2">
        ...
        <input className="h-8 w-full rounded-act-sm border border-line bg-surface pl-7 pr-2 text-[12px] text-text-main outline-none placeholder:text-text-faint focus:border-focus-ring" value={filter} onChange={(event) => onFilter(event.target.value)} placeholder="筛选文件…" aria-label="筛选变更文件" />
        </div>
      ...
      <div className="min-h-0 flex-1 overflow-auto px-1 pb-2" role="tree">
        ...
        {filtered.length === 0 ? <div className="px-3 py-6 text-center text-[12px] text-text-faint">没有匹配的文件。</div> : null}
      </div>
    </aside>
  );
}
