 ...
 * 工具清单单一来源。
 *
 * 由「工具」分区用于主 Agent 的全局开关，
 * 避免两处各维护一份工具名而漂移。普通项的 `name` 与 Runtime Tool Registry 注册名一致；
...
export const TOOL_ITEMS: ToolCatalogItem[] = [
  ...
  { name: "inspect_image", label: "图片分析", description: "委托已配置的多模态模型分析本地图片。", conditional: true },
