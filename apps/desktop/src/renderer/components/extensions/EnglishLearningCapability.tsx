...
const button = "inline-flex h-8 items-center gap-1.5 rounded-act-md border border-line bg-surface px-3 text-[12px] text-text-main hover:bg-hover-overlay focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-ring disabled:opacity-50";
    ...
    <section aria-label="英语辅助学习" className="overflow-hidden rounded-act-lg bg-surface-subtle">
      <div className="flex items-center gap-3 px-4 py-4">
        <button type="button" aria-expanded={expanded} aria-controls="english-learning-details" onClick={() => setExpanded(!expanded)} className="flex min-w-0 flex-1 items-center gap-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-ring">
          ...
          <span className="min-w-0 flex-1"><span className="block text-[13px] font-medium text-text-main">英语辅助学习</span><span className="mt-0.5 block text-[12px] text-text-faint">让选定会话按英文、中文对照回答，并朗读英文。</span></span>
      ...
      {(expanded || error) && <div id="english-learning-details" className="flex flex-col gap-4 border-t border-line px-4 py-4">
        {!available ? <p className="text-[12px] text-text-faint">英语辅助学习仅在桌面端可用。</p> : <>
          <label className="flex flex-col gap-2 text-[12px] text-text-muted">目标会话
          ...
          <p role="status" className="text-[12px] text-text-faint">{!state ? "加载中…" : state.enabled ? `双语已开启 · ${SPEECH_STATUS_LABELS[state.speechStatus]}` : "已关闭 · 开启后从下一次模型请求生效"}{state && state.queuedSegments > 0 ? ` · 待播 ${state.queuedSegments} 段` : ""}</p>
          {state?.notice && <p className="text-[12px] text-text-faint">{state.notice}</p>}
          {state?.error && <p role="alert" className="text-[12px] text-on-danger">{state.error.message}</p>}
          ...
          <p className="text-[11px] leading-relaxed text-text-faint">只影响选定会话。停止播放会保留双语模式，历史回答不会补播。</p>
        </>}
        {error && <p role="alert" className="text-[12px] text-on-danger">{error}</p>}
      </div>}
    </section>
  );
}
