...
export type AgentAnalysisSessionIndexViewState = {
  index: AgentAnalysisSessionIndexResult;
  hasLoaded: boolean;
  query: string;
  statusFilter: "all" | AgentAnalysisSessionStatus;
  modelFilter: string;
  scrollTop: number;
};

export function createAgentAnalysisSessionIndexViewState(): AgentAnalysisSessionIndexViewState {
  return {
    index: EMPTY_SESSION_INDEX,
    hasLoaded: false,
    query: "",
    statusFilter: "all",
    modelFilter: "all",
...
export function AgentAnalysisSessionIndex({
  activeSessionId,
  state,
  onStateChange,
  onOpenSession,
  ...
  state: AgentAnalysisSessionIndexViewState;
  onStateChange: Dispatch<SetStateAction<AgentAnalysisSessionIndexViewState>>;
  onOpenSession: (sessionId: string) => void;
}) {
  const [loading, setLoading] = useState(!state.hasLoaded);
  const [error, setError] = useState<string | null>(null);
  const scrollRef = useRef<HTMLDivElement>(null);
  const { index, query, statusFilter, modelFilter } = state;

  useEffect(() => {
    const scrollContainer = scrollRef.current;
    if (scrollContainer) scrollContainer.scrollTop = state.scrollTop;
  }, []);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(!state.hasLoaded);
    setError(null);
    window.actspace.getAgentAnalysisSessionIndex()
      .then((next) => {
        if (!cancelled) {
          onStateChange((current) => ({ ...current, index: next, hasLoaded: true }));
        }
      })
  ...
  const updateFilters = (patch: Partial<Pick<AgentAnalysisSessionIndexViewState, "query" | "statusFilter" | "modelFilter">>) => {
    if (scrollRef.current) scrollRef.current.scrollTop = 0;
    onStateChange((current) => ({ ...current, ...patch, scrollTop: 0 }));
  };
  ...
  return (
    <div
      ...
      data-testid="agent-analysis-session-index"
      onScroll={(event) => {
        const scrollTop = event.currentTarget.scrollTop;
        onStateChange((current) => current.scrollTop === scrollTop ? current : { ...current, scrollTop });
      }}
    >
      <div className="mx-auto w-full max-w-[1180px] px-8 pb-10 pt-8 max-[600px]:px-4 max-[600px]:pb-6 max-[600px]:pt-5">
        <div className="flex min-h-9 items-center gap-2">
          <h2 className="text-[22px] font-bold text-text-main">分析观测</h2>
        ...
        <section aria-label="分析会话列表" className="mt-6 overflow-hidden rounded-act-lg border border-line bg-surface shadow-act-soft">
          <div className="flex items-center gap-2 border-b border-line bg-surface px-3 py-3 max-[720px]:flex-wrap">
            ...
            <label className="w-[148px] max-[720px]:min-w-0 max-[720px]:flex-1">
              ...
              <select
                value={statusFilter}
                onChange={(event) => updateFilters({ statusFilter: event.target.value as "all" | AgentAnalysisSessionStatus })}
            ...
            <label className="w-[172px] max-[720px]:min-w-0 max-[720px]:flex-1">
              ...
              <select
                value={modelFilter}
                onChange={(event) => updateFilters({ modelFilter: event.target.value })}
                className="h-[38px] w-full cursor-pointer rounded-act-md border border-line bg-surface px-3 text-[12px] text-text-main outline-none focus:border-line-strong focus:ring-2 focus:ring-focus-ring"
              >
