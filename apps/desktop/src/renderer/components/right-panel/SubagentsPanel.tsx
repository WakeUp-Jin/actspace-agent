...
export function SubagentsPanel({ sessionId, selected }: { sessionId: string | null; selected?: AgentMessage }) {
  ...
  if (!sessionId) return <p className="p-5 text-act-sm text-text-muted">选择会话查看子 Agent。</p>;
  if (active) return <section className="flex h-full min-h-0 flex-col" aria-label="子 Agent 详情">
    <header className="flex shrink-0 items-center gap-3 border-b border-line px-4 py-3">
      ...
      <div className="min-w-0 flex-1"><h2 className="m-0 truncate text-act-sm font-medium" title={active.description}>{active.description}</h2><p className="m-0 mt-1 flex items-center gap-2 text-act-xs text-text-muted"><span>{kindLabel(active)}</span><span aria-hidden="true">·</span><AgentStatus status={active.status} /></p></div>
    </header>
    <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4" aria-label="子 Agent 执行内容">
      {error ? <p role="status" className="text-act-xs text-text-muted">{error}</p> : null}
      {messages.map((message) => renderMessage(message))}
      {!messages.length ? <p className="text-act-sm text-text-muted">{active.summary || "等待执行…"}</p> : null}
    </div>
  </section>;
  return <section className="h-full overflow-y-auto p-4" aria-label="子 Agent 列表">
    ...
    {([true, false] as const).map((running) => {
      ...
      return <div key={String(running)} className="mb-6"><h2 className="mb-2 px-3 text-act-xs font-normal text-text-faint">{running ? "运行中" : "已结束"} · {items.length}</h2>
        {items.map((agent) => <button key={agent.id} type="button" className={buttonClass} onClick={() => setSelection(agent)}>
          <span className="flex items-center gap-3">
            ...
            <span className="min-w-0 flex-1">
              <span className="block truncate text-act-sm text-text-main" title={agent.description}>{agent.description}</span>
              <span className="mt-1 flex items-center gap-1.5 text-act-xs text-text-muted">
    ...
    {!agents.length && !error ? <p className="px-3 text-act-sm text-text-muted">此会话还没有子智能体。</p> : null}
  </section>;
}
