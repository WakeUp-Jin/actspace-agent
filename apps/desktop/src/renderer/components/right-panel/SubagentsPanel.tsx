...
import { ArrowLeft, Check, Loader2, CircleAlert } from "lucide-react";
...
export function SubagentsPanel({ sessionId, selected }: { sessionId: string | null; selected?: AgentMessage }) {
  ...
  if (!sessionId) return <p className="p-5 text-[13px] text-text-muted">选择会话查看子智能体。</p>;
  if (active) return <section className="flex h-full min-h-0 flex-col" aria-label="Subagent details">
    <header className="flex shrink-0 items-center gap-3 border-b border-line px-4 py-3">
      <button type="button" aria-label="Back to subagents" className="rounded-act-md p-1.5 hover:bg-surface-subtle focus-visible:outline focus-visible:outline-2 focus-visible:outline-focus-ring" onClick={() => setSelection(undefined)}><ArrowLeft size={16} /></button>
    ...
    <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4" aria-label="Subagent execution">
      {error ? <p role="status" className="text-[12px] text-text-muted">{error}</p> : null}
      {messages.map((message) => renderMessage(message))}
      {!messages.length ? <p className="text-[13px] text-text-muted">{active.summary || "Waiting for execution…"}</p> : null}
    </div>
  </section>;
  return <section className="h-full overflow-y-auto p-4" aria-label="Subagents list">
    {error ? <p role="status" className="text-[12px] text-text-muted">{error}</p> : null}
    {([true, false] as const).map((running) => {
      const items = agents.filter((agent) => (agent.status === "running") === running);
      if (!items.length) return null;
      return <div key={String(running)} className="mb-6"><h2 className="mb-2 px-3 text-[12px] font-normal text-text-faint">{running ? "Running" : "Done"} · {items.length}</h2>
        {items.map((agent) => <button key={agent.id} type="button" className={buttonClass} onClick={() => setSelection(agent)}>
          <span className="flex items-start gap-3">
            <span className="mt-0.5 text-text-muted">{running ? <Loader2 size={17} className="animate-spin motion-reduce:animate-none" /> : agent.status === "completed" ? <Check size={17} /> : <CircleAlert size={17} />}</span>
            <span className="min-w-0 flex-1"><span className="block truncate text-[13px] text-text-main" title={agent.description}>{agent.description}</span><span className="mt-1 block text-[12px] text-text-muted">{statusLabel(agent)}</span></span>
            <span className="shrink-0 text-[12px] text-text-faint">{Math.floor((agent.stats?.durationMs ?? 0) / 1000)}s</span>
          </span>
        </button>)}
      </div>;
    })}
...
function statusLabel(agent: AgentMessage): string { return agent.status === "running" ? "Working" : agent.status === "completed" ? "Completed" : agent.status === "aborted" ? "Stopped" : "Failed"; }
