...
export const homeFeatures: Feature[] = [
  ...
  { id:'execution', label:'工具执行', title:'从读取项目，到修改与验证', description:'把目标交给 Agent，在同一个会话里跟进文件读取、修改和命令执行，直到拿到可检查的结果。', points:['读取与检索工作区文件','查看文件生成进度与实际改动','检查命令输出，在需要时批准操作'], href:'/docs/files-and-search/', image:'tool-execution.png', imageTitle:'项目探索中的文件读取、目录浏览与命令执行记录' },
  ...
  { id:'subagents', label:'子 Agent', title:'把资料检索交给子 Agent', description:'把需要独立探索的问题拆出去。主会话保留任务全貌，右侧随时查看子任务的活动与发现。', points:['只读地读取文件与搜索项目','跟进正在读取、搜索和整理的状态','打开独立详情，检查结果和失败原因'], href:'/docs/subagents/', image:'subagent-running.png', imageTitle:'只读子任务与运行状态' },
  { id:'trajectory', label:'会话轨迹', title:'回到每一次请求与工具调用', description:'从聊天切换到 Trajectory，沿时间线检查执行记录。遇到问题时，找到对应的请求、参数与结果。', points:['搜索并定位会话中的执行记录','检查工具参数和返回结果','保留聊天草稿，随时切回任务'], href:'/docs/agent-turn/', image:'trajectory-inspector.png', imageTitle:'会话时间线与用户消息详情' },
];
